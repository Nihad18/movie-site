...
const DatePickerHeader = ({
    ...
    <div style={{ margin: 10, display: "flex", justifyContent: "space-between" }}>
        ...
        <select value={getYear(date)} onChange={(e) => changeYear(parseInt(e.target.value))} className='dataPicker-select'>
            {years.map((option) => (
        ...
        <select value={months[getMonth(date)]} onChange={(e) => changeMonth(months.indexOf(e.target.value))} className='dataPicker-select'>
            {months.map((option) => (
                <option key={option} value={option}>
                    {option}
                </option>
